...
func NewInstallCmd(c CmdOpts) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "install",
		Short: "Helper command for setting up k0s on a brand-new system. Must be run as root (or with sudo)",
	}
	cmd.AddCommand(installControllerCmd(&c))
	cmd.AddCommand(installWorkerCmd(&c))
...
func (c *CmdOpts) preRunValidateConfig(cmd *cobra.Command, args []string) error {
	_, err := config.ValidateYaml(c.CfgFile, c.K0sVars.DataDir)
	if err != nil {
		return err
	}
	return nil
