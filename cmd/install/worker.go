...
func installWorkerCmd(c *CmdOpts) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "worker",
...
Windows flags like "--api-server", "--cidr-range" and "--cluster-dns" will be ignored since install command doesn't yet support Windows services`,
	...
	workerFlagset := c.getWorkerFlags()
	cmd.Flags().AddFlagSet(workerFlagset)

	return cmd
}

func (c *CmdOpts) getWorkerFlags() *pflag.FlagSet {
	cmdOpts := config.CLIOptions(*c)
	return worker.NewWorkerCmd(worker.CmdOpts(cmdOpts)).Flags()
}
