...
func installControllerCmd(c *CmdOpts) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "controller",
...
With controller subcommand you can setup a single node cluster by running:
	...
	controllerFlagSet := c.getControllerFlags()
	cmd.Flags().AddFlagSet(controllerFlagSet)

	return cmd
}

func (c *CmdOpts) getControllerFlags() *pflag.FlagSet {
	cmdOpts := config.CLIOptions(*c)
	return controller.NewControllerCmd(controller.CmdOpts(cmdOpts)).Flags()
}
